...
export default class InfoPage extends Component {
    ...
    render() {
        ...
        const infoData = data ?
            ...
            []
        const card_image = [];
        const next_image = []; //传到图片放大组件页面的数据
        const foot = [];
        if (data.card_face) {
            card_image.push({ uri: data.card_face })
            next_image.push({ url: data.card_face })
            foot.push('证件照正面')
        }
        if (data.card_identity) {
            card_image.push({ uri: data.card_identity })
            next_image.push({ url: data.card_identity })
            foot.push('证件照背面')
        }
        return (
            <View style={{ flex: 1 }}>
                <View style={[styles.flex_row_columncenter, { padding: 5 }]}>
                    {card_image.map((item, index) => {
                        return (
                            <TouchableOpacity
                                key={index}
                                activeOpacity={1}
                                style={[styles.flex_center, { flex: 0.5 }]}
                                onPress={() => {
                                    this.props.navigation.navigate('ImageView', { data: next_image, foot: foot, num: index })
                                }}
                            >
                                <Image source={item} style={{ width: 100, height: 70 }} />
                            </TouchableOpacity>
                        )
                    })
                    }
                ...
                <FlatList
                    ...
                    ListHeaderComponent={() => <View style={{ borderColor: '#ccc', borderTopWidth: 0.5 }}></View>}
                />
            </View>
        )
    }
